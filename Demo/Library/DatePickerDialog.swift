...
class DatePickerDialog: UIView {
    ...
    typealias DateRangePickerCallback = (dateFrom: NSDate, dateUpTo: NSDate) -> Void
    ...
    private let kDatePickerHeight:                          CGFloat = 230
    ...
    private var titleLabelUpTo: UILabel!
    private var datePickerUpTo: UIDatePicker!

    /* Vars */
    private var isRangePicker: Bool
    private var defaultDate:    NSDate?
    private var datePickerMode: UIDatePickerMode?
    private var callback:       DatePickerCallback?
    private var rangeCallback:       DateRangePickerCallback?


    /* Overrides */
    init(isRangePicker: Bool) {
        self.isRangePicker = isRangePicker
    ...
    func show(title: String, doneButtonTitle: String = "Done", cancelButtonTitle: String = "Cancel", defaultDate: NSDate = NSDate(), datePickerMode: UIDatePickerMode = .DateAndTime, callback: DatePickerCallback) {
        ...
        self.datePicker.date = defaultDate ?? NSDate()
        self.callback = callback
    }

    func show(title: String, titleUpTo: String = "", doneButtonTitle: String = "Done", cancelButtonTitle: String = "Cancel", defaultDate: NSDate = NSDate(), defaultDateUpTo: NSDate = NSDate(), datePickerMode: UIDatePickerMode = .DateAndTime, callback: DateRangePickerCallback) {

        if(!isRangePicker){
            print("!!!!! this show() method is for range picker !!!!!")
            return
        }
        ...
        self.titleLabelUpTo.text = titleUpTo
        self.datePickerUpTo.datePickerMode = datePickerMode ?? .Date
        self.datePickerUpTo.date = defaultDateUpTo ?? NSDate()
        self.rangeCallback = callback

        doShow(doneButtonTitle, cancelButtonTitle: cancelButtonTitle)
    }

    private func doShow(doneButtonTitle: String = "Done", cancelButtonTitle: String = "Cancel"){
    ...
    private func createContainerView() -> UIView {
        ...
        let pickerAreaHeight = isRangePicker ? kDatePickerHeight * 2 : kDatePickerHeight
        ...
        //Title
        self.titleLabel = UILabel(frame: CGRectMake(10, 10, 280, kLabelHeight))
        self.titleLabel.textAlignment = NSTextAlignment.Center
        self.titleLabel.font = UIFont.boldSystemFontOfSize(17)
        dialogContainer.addSubview(self.titleLabel)

        self.datePicker = UIDatePicker(frame: CGRectMake(0, kLabelHeight, 0, 0))
        ...
        if(isRangePicker){
            //Title
            self.titleLabelUpTo = UILabel(frame: CGRectMake(10, 10 + kDatePickerHeight, 280, kLabelHeight))
            self.titleLabelUpTo.textAlignment = NSTextAlignment.Center
            self.titleLabelUpTo.font = UIFont.boldSystemFontOfSize(17)
            dialogContainer.addSubview(self.titleLabelUpTo)

            self.datePickerUpTo = UIDatePicker(frame: CGRectMake(0, kDatePickerHeight + kLabelHeight, 0, 0))
            self.datePickerUpTo.autoresizingMask = UIViewAutoresizing.FlexibleRightMargin
            self.datePickerUpTo.frame.size.width = 300
            dialogContainer.addSubview(self.datePickerUpTo)
        }
    ...
    func buttonTapped(sender: UIButton!) {
        if sender.tag == kDatePickerDialogDoneButtonTag {
            if(isRangePicker){
                self.rangeCallback?(dateFrom: self.datePicker.date, dateUpTo: self.datePickerUpTo.date)
            }else{
